...
import {
  ...
  ExternalLink, CheckCircle2, ChevronLeft, ChevronRight, Maximize, RotateCw,
...
const LectureRoom = () => {
  ...
  return (
    <div className="bg-mesh animate-fade-in" style={{ minHeight: '90vh', paddingBottom: '5rem' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', overflow: 'hidden' }}>
          ...
          <div style={{ overflow: 'hidden' }}>
            <span style={{ fontSize: '0.68rem', color: '#38BDF8', fontWeight: 800, textTransform: 'uppercase', display: 'block' }}>
              PiyushDhara Study Room
            </span>
            <h2 style={{ fontSize: '0.9rem', fontWeight: 800, color: '#FFFFFF', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {video.title}
            </h2>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
          <button
            ...
            style={{
              background: studyMode ? '#1E293B' : 'rgba(255,255,255,0.1)',
          ...
          >
            {studyMode ? <Sun size={16} color="#F59E0B" /> : <Moon size={16} color="#38BDF8" />}
          </button>

          <img
            src={userObj?.photo || '/gaurov.jpeg'}
            onError={(e) => { e.target.src = '/gaurov.jpeg'; }}
            alt="Profile"
            style={{ width: '34px', height: '34px', borderRadius: '50%', border: '2px solid #2563EB', objectFit: 'cover' }}
          />
        </div>
      </div>
      ...
      <div className="container" style={{ paddingTop: '1.25rem' }}>
        ...
        {/* ── 1. COMPACT BREADCRUMB & SMALL MOBILE STUDY MODE ICON (ABOVE VIDEO) ── */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
          <div className="lecture-breadcrumb" style={{ margin: 0, fontSize: '0.85rem' }}>
            <Link to="/courses">Batches</Link>
            <span className="sep">›</span>
            <Link to={`/courses/${video.chapter || ''}`}>Course</Link>
            <span className="sep">›</span>
            <span style={{ color: 'var(--text-primary)', fontWeight: 600, maxWidth: '280px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {video.title}
            </span>
          </div>

          {/* Dark Theme / Study Mode Toggle Pill Button (Mobile & Desktop Visible) */}
          <button
            type="button"
            onClick={toggleStudyMode}
            style={{
              background: studyMode ? 'linear-gradient(135deg, #0F172A 0%, #1E293B 100%)' : 'linear-gradient(135deg, #EFF6FF 0%, #DBEAFE 100%)',
              border: studyMode ? '1.5px solid #38BDF8' : '1.5px solid #2563EB',
              color: studyMode ? '#38BDF8' : '#1D4ED8',
              borderRadius: '9999px',
              ...
              gap: '0.45rem',
              cursor: 'pointer',
              boxShadow: studyMode ? '0 4px 14px rgba(56, 189, 248, 0.25)' : '0 4px 14px rgba(37, 99, 235, 0.15)',
              transition: 'all 0.2s ease',
              flexShrink: 0
            }}
            title="Toggle Dark Theme / Study Mode"
          >
            {studyMode ? <Sun size={15} color="#F59E0B" /> : <Moon size={15} color="#2563EB" />}
            <span>{studyMode ? '🌙 Dark Mode: ON' : '💡 Dark Mode: OFF'}</span>
          </button>
        </div>

        {/* Two-column layout (stacks vertically on mobile) */}
        <div className="main-content-grid lecture-main-grid" style={{
          display: 'grid',
          ...
          {/* ── Left: Player + Info ── */}
          <div className="lecture-info-panel" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>

            {/* ── 2. VIDEO PLAYER CONTAINER (16:9 Aspect Ratio) ── */}
            <div ref={videoContainerRef} className="lecture-video-container aspect-ratio-16-9" style={{ position: 'relative', overflow: 'hidden' }}>
              {ytId ? (
                <iframe
              ...
              {/* Fullscreen & Rotate Overlay Button (Mobile Friendly) */}
              <button
                type="button"
                ...
                title="Full Screen & Rotate Mobile Video"
              >
                <Maximize size={13} />
                <RotateCw size={13} color="#38BDF8" />
            ...
            {!ytId && (
              <div className="card" style={{ padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.65rem', flexWrap: 'wrap' }}>
                <Zap size={15} color="var(--warning)" />
                <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-muted)' }}>Playback Speed:</span>
                {[0.75, 1, 1.25, 1.5, 1.75, 2].map((speed) => (
                  <button
                    key={speed}
                    onClick={() => setPlaybackSpeed(speed)}
                    style={{
                      padding: '0.25rem 0.6rem', fontSize: '0.8rem', fontWeight: 600,
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.65rem', flexWrap: 'wrap' }}>
                ...
                <span className="badge badge-blue">🇳🇵 SEE &amp; NEB Nepal</span>
              </div>
            ...
            {/* ── 5. PRIMARY CTA BUTTON & SECONDARY ACTION GRID ── */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              
              {/* Primary CTA: Watch on YouTube / Stream */}
              {ytId && (
                <a
                  href={video.videoUrl}
                  target="_blank"
              ...
              {/* Secondary Action Grid (2 columns on mobile) */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: isBookmarked ? 'var(--primary)' : 'var(--text-primary)',
                    background: isBookmarked ? 'var(--primary-light)' : 'var(--bg-card)',
                  }}
                >
            ...
            {/* ── MOBILE-ONLY: STREAK & HANDOUTS / PLAYLIST CARD (ABOVE DISCUSSION ON MOBILE PHONES) ── */}
            <div className="mobile-only-resources" style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', marginTop: '1rem' }}>
              
              {/* Daily Study Streak Pill Badge */}
              <div 
              ...
              {/* Mobile Handouts & Playlist Tabs Card */}
              <div className="card" style={{
                ...
                <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg-input)' }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '0.75rem', fontSize: '0.85rem', fontWeight: 800, justifyContent: 'center' }}
                  >
                  ...
                  {activeTab === 'notes' ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                      {chapterNotes.length === 0 ? (
                        <div style={{ padding: '1rem', border: '1px solid var(--border)', background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.85rem' }}>
                            <div style={{ width: '38px', height: '38px', borderRadius: '0.75rem', background: 'rgba(239, 68, 68, 0.1)', color: '#EF4444', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <FileText size={20} />
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: 0 }}>
                                PDF Handout &amp; Formula Notes prepared by Gaurav Sir.
                              </p>
                            </div>
                          </div>
                          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                            <a
                              href={video?.notePdfUrl ? getFileUrl(video.notePdfUrl) : '#'}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="btn btn-primary"
                              style={{ flex: 1, padding: '0.55rem', fontSize: '0.8rem', gap: '0.35rem', justifyContent: 'center' }}
                            >
                        ...
                        chapterNotes.map((note) => {
                          const notePdfUrl = note.pdfUrl ? getFileUrl(note.pdfUrl) : '#';
                          return (
                            <div key={note._id || note.title} style={{ padding: '1rem', border: '1px solid var(--border)', background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.85rem' }}>
                                <div style={{ width: '38px', height: '38px', borderRadius: '0.75rem', background: 'rgba(239, 68, 68, 0.1)', color: '#EF4444', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                  <FileText size={20} />
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: 0 }}>
                                    {note.description || 'PDF Handout & Formula Notes'}
                                  </p>
                                </div>
                              </div>
                              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                <a href={notePdfUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ flex: 1, padding: '0.55rem', fontSize: '0.8rem', gap: '0.35rem', justifyContent: 'center' }}>
                                  <ExternalLink size={14} /> Open in New Tab
                                </a>
                                <a href={notePdfUrl} download className="btn btn-outline" style={{ flex: 1, padding: '0.55rem', fontSize: '0.8rem', gap: '0.35rem', justifyContent: 'center' }}>
                                  <Download size={14} /> Download PDF
                                </a>
                              </div>
                            </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                      {chapterVideos.length === 0 ? (
                        <div className="empty-state">
                          ...
                          <p style={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--text-primary)', margin: 0 }}>No playlist</p>
                        </div>
                      ) : (
                          ...
                          return (
                            <Link
                              ...
                              style={{
                                ...
                                padding: '0.65rem 0.75rem', borderRadius: 'var(--radius-lg)',
                                textDecoration: 'none',
                                background: isCurrent ? 'linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%)' : 'var(--bg-input)',
                                border: isCurrent ? 'none' : '1px solid var(--border)',
                              }}
                            >
                              <span style={{
                                width: '22px', height: '22px', borderRadius: '50%', flexShrink: 0,
                                background: isCurrent ? 'rgba(255,255,255,0.2)' : 'var(--bg-card)',
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <p style={{ fontSize: '0.82rem', fontWeight: isCurrent ? 700 : 500, margin: 0, color: isCurrent ? '#FFFFFF' : 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.title}</p>
                              </div>
                            </Link>
                          );
                        })
          ...
          {/* ── Right: Resources Panel (DESKTOP ONLY) ── */}
          <div className="lecture-resources-panel desktop-only-sidebar" style={{ position: 'sticky', top: 'calc(var(--navbar-height) + 1rem)', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
      ...
      >
        <button
          onClick={() => setBottomTab('discussion')}
          style={{
            background: bottomTab === 'discussion' ? 'rgba(37,99,235,0.2)' : 'none',
            border: 'none',
            color: bottomTab === 'discussion' ? '#38BDF8' : '#94A3B8',
        ...
        <button
          onClick={() => setActiveTab('notes')}
          style={{
            background: activeTab === 'notes' ? 'rgba(37,99,235,0.2)' : 'none',
            border: 'none',
        ...
        <button
          onClick={() => setActiveTab('playlist')}
          style={{
            background: activeTab === 'playlist' ? 'rgba(37,99,235,0.2)' : 'none',
            border: 'none',
            color: activeTab === 'playlist' ? '#38BDF8' : '#94A3B8',
            padding: '0.5rem 0.2rem',
            borderRadius: '0.65rem',
            display: 'flex',
            flexDirection: 'column',
        ...
        <button
          onClick={() => setBottomTab('feedback')}
          style={{
            background: bottomTab === 'feedback' ? 'rgba(245,158,11,0.2)' : 'none',
            border: 'none',
            color: bottomTab === 'feedback' ? '#F59E0B' : '#94A3B8',
            padding: '0.5rem 0.2rem',
            borderRadius: '0.65rem',
            display: 'flex',
            flexDirection: 'column',
        ...
        >
          <Star size={18} fill={bottomTab === 'feedback' ? "#F59E0B" : "none"} />
          <span>Rating</span>
        </button>
